...
const Benefits = () => {
  return (
    <div className="bg-black text-center px-4 sm:px-6">
      <motion.button
      ...
      <div className="max-w-6xl mx-auto mt-8 sm:mt-12">
        <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {benefitss.map((benefits, index) => (
            <div
              key={index}
              className="relative border border-gray-800 rounded-lg p-4 sm:p-4 transition-colors duration-300 group bg-black before:content-[''] before:absolute before:inset-x-0 before:bottom-0 before:h-24 before:bg-gradient-to-t before:from-blue-500/30 before:to-transparent before:blur-2xl before:rounded-b-lg"
            >
              <div className="text-white mb-4 sm:mb-6 group-hover:text-blue-400 transition-colors duration-300">
                {benefits.icon}
              </div>
              <p className="text-white text-base sm:text-lg font-semibold mb-3 sm:mb-4 text-left leading-tight">
                {benefits.title}
              </p>
